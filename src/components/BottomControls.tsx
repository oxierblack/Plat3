...
export default function BottomControls() {
  ...
  const openTrade = useCallback(async (side: 'buy' | 'sell') => {
    ...
    setTimeout(async () => {
      ...
      // Strict inequality: exact break-even price = loss (not a win)
      const won = side === 'buy' ? exitPrice > trade.entry : exitPrice < trade.entry;
  ...
  }, [currentMarket, amount, bal, walType, expMin, expDisp, payout, soundEnabled]);
