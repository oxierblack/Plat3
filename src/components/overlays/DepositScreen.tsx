      ...
      <div className="fs-body" style={{ paddingTop: step === 'done' ? 32 : 16 }}>
        ...
        {step === 'select' && (
          <>
            <div style={{ display:'flex', background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--r2)', padding:4, gap:4, marginBottom:16 }}>
              {([['egy','المحافظ المصرية'],['crypto','العملات الرقمية']] as [DepTab,string][]).map(([t, label]) => (
                  ...
                  {EGY_WALLETS.map(w => (
                    <button key={w.id} onClick={() => { setWallet(w); setStep('form'); }}
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:'var(--t4)', marginTop:2 }}>Min 1,000 EGP · 0% رسوم</div>
                      </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            {!isCrypto && (
              <div className="auth-field">
                ...
                />
                <div style={{ fontSize:11, color:'var(--t4)', marginTop:2 }}>ابدأ بـ +2 (مثال: +201001234567)</div>
              </div>
            )}

            {/* Amount */}
            <div className="auth-field">
              <label>المبلغ / Amount ({currency})</label>
            ...
            <div className="auth-field">
              <label style={{ display:'flex', alignItems:'center', gap:6 }}>
                كود البونص / Bonus Code
                <span style={{ fontSize:10, color:'var(--t4)', fontWeight:600, background:'var(--bg3)', padding:'2px 6px', borderRadius:4 }}>اختياري</span>
              </label>
            ...
            {amount && parseFloat(amount) >= minAmount && (
              <div style={{ padding:'12px 14px', background:'rgba(0,214,143,.06)', border:'1px solid rgba(0,214,143,.18)', borderRadius:'var(--r2)', fontSize:13 }}>
                <div style={{ color:'var(--t3)', marginBottom:4 }}>سترسل / You will send:</div>
        ...
        {step === 'payment' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ padding:'12px 14px', background:'rgba(59,130,246,.06)', border:'1px solid rgba(59,130,246,.18)', borderRadius:'var(--r2)', fontSize:13, color:'var(--t3)', lineHeight:1.7 }}>
              {isCrypto
                ? <>أرسل <strong style={{ color:'var(--t1)' }}>{amount} {currency}</strong> إلى العنوان أدناه، ثم ارفع إيصال التحويل.</>
                : <>حوّل <strong style={{ color:'var(--t1)' }}>{parseFloat(amount).toLocaleString()} EGP</strong> من <strong style={{ color:wallet?.color }}>{wallet?.name}</strong> إلى الرقم أدناه، ثم ارفع صورة الإيصال.</>
              }
            </div>
            ...
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:'var(--r2)', padding:16 }}>
              <div style={{ fontSize:11, color:'var(--t4)', fontWeight:700, letterSpacing:'.5px', textTransform:'uppercase', marginBottom:10 }}>
                {isCrypto ? 'Wallet Address' : 'رقم التحويل / Transfer To'}
              </div>
        ...
        {step === 'done' && (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <div style={{ fontSize:22, fontWeight:900, color:'var(--t1)', marginBottom:8 }}>تم الإرسال!</div>
            <div style={{ fontSize:22, fontWeight:700, color:'var(--t1)', marginBottom:4 }}>Receipt Submitted</div>
            <div style={{ fontSize:13, color:'var(--t3)', lineHeight:1.8, marginBottom:20 }}>
              طلبك قيد المراجعة من فريقنا.<br/>
