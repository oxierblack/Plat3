          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            {bonusBalance > 0 && (
              <div style={{ background:'rgba(255,61,87,.06)', border:'1px solid rgba(255,61,87,.2)', borderRadius:'var(--r2)', padding:12, fontSize:12, color:'var(--red)' }}>
                ...
                <br/>Withdrawable balance: <strong>${Math.max(0, realBalance - bonusBalance).toFixed(2)}</strong>
              </div>
            )}
            <div style={{ background:'rgba(245,158,11,.06)', border:'1px solid rgba(245,158,11,.2)', borderRadius:'var(--r2)', padding:12, fontSize:12, color:'#F59E0B' }}>
              Real balance: <strong>${realBalance.toFixed(2)}</strong>
              {bonusBalance > 0 ? <> · Withdrawable: <strong>${Math.max(0, realBalance - bonusBalance).toFixed(2)}</strong></> : null}
