...
export default function TransfersScreen() {
  ...
  const withdrawMethods = [
    { id:'vodafone', label:'Vodafone Cash', color:'#E60000', icon:'phone' },
    { id:'instapay', label:'InstaPay',      color:'#0066CC', icon:'bank'  },
    { id:'bank',     label:'Bank Wire',     color:'#64748B', icon:'bank'  },
    { id:'usdt',     label:'USDT TRC20',    color:'#26A17B', icon:'coin'  },
    { id:'btc',      label:'Bitcoin',       color:'#F7931A', icon:'coin'  },
  ];
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div className="withdraw-method-grid">
                {withdrawMethods.map(m => (
                  <button
                    key={m.id}
                    type="button"
                    className={`withdraw-method-card${method === m.id ? ' active' : ''}`}
                    onClick={() => setMethod(m.id)}
                  >
                    <div className="withdraw-method-ico" style={{ background: `${m.color}20`, color: m.color }}>
                      {m.icon === 'coin'
                        ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9"/><path d="M12 7v10M9 9.5c0-1.2 1.3-2 3-2s3 .9 3 2-1.3 1.5-3 2-3 .8-3 2 1.3 2 3 2 3-.8 3-2"/></svg>
                        : m.icon === 'bank'
                        ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 21h18M4 10h16M6 10V21M18 10V21M10 10V21M14 10V21M12 3L3 8h18L12 3z"/></svg>
                    ...
                    <span className="withdraw-method-label">{m.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="auth-field">
              ...
              <div className="quick-amount-row">
                {[25, 50, 100, Math.max(10, Math.floor(Math.max(0, realBalance - bonusBalance)))].map((v, i) => (
                  <button
                    key={i}
                    type="button"
